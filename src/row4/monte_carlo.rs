...
pub fn choose_next_move(board: &Board, own_color: Color, color_to_move: Color, millis: u32) -> (Column, f64, u32) {
...
pub fn evaluate(board: &Board, own_color: Color, color_to_move: Color, millis: u32) -> (f64, u32) {
    ...
    while millis_since(start) < millis {
        let mut sim = board.clone();
        match play_random_game(&mut sim, color_to_move.clone()) {
...
fn useful_moves(board: &Board, color_to_move: Color) -> Vec<Column> {
